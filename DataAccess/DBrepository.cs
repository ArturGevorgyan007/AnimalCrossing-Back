...
public class DBRepository : IRepository
{
    ...
    public List<User> GetAllUsers()
    {
        ...
        using SqlCommand cmd = new SqlCommand("SELECT * FROM USERS order by wallet Desc", connection);
        ...
        while(reader.Read()) {
            ...
            userList.Add(new User(uid,uFName,uLName,uName,uPassword,uWallet));         
        }
        return userList;
    }
    ...
    public List<Marketplace> GetMarketplaceItems() {

        List<Marketplace> listingList = new List<Marketplace>();
        using SqlConnection connection = new SqlConnection(_connectionString); 
        
        connection.Open();

        using SqlCommand cmd = new SqlCommand("SELECT STORE_INVENTORY.listing_id,ITEM.name, STORE_INVENTORY.quantity, STORE_INVENTORY.unit_price, ITEM.url, STORE_INVENTORY.seller_id  FROM STORE_INVENTORY join ITEM on ITEM.id=STORE_INVENTORY.item_id", connection);
        using SqlDataReader reader = cmd.ExecuteReader();
        int i =0;
        while(reader.Read()) {
            ...
            string iurl=(string) reader["url"];
            int isellerid = (int) reader["seller_id"];
            // Console.WriteLine(iname);
            listingList.Add(new Marketplace(iid,iname,iquantity,iprice,iurl,isellerid));
            }            
        
        return listingList;
    }

    public List<Marketplace> getMarketplaceItemsByName(string searchitem) {
        ...
        using SqlCommand cmd = new SqlCommand($"SELECT STORE_INVENTORY.listing_id,ITEM.name, STORE_INVENTORY.quantity, STORE_INVENTORY.unit_price, ITEM.url, STORE_INVENTORY.seller_id FROM STORE_INVENTORY join ITEM on ITEM.id=STORE_INVENTORY.item_id where ITEM.name LIKE '%{searchitem}%'", connection);
        using SqlDataReader reader = cmd.ExecuteReader();
