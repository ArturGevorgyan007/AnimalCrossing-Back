...
app.MapGet("/leaderboard", (UserServices service) => {
    try{
        Log.Information("Attempting to get all users sorted by wallet desc for leaderboard...");
        return service.GetUsers();
    }
    catch(Exception ex){
        Log.Error("Something fatal happened when getting users sorted by wallet desc for leaderboard", ex);
        throw;
    }
});
